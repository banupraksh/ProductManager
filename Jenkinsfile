    ...
    stage('Push Docker Image'){
     ...
     stage('Deploy on Docker Container'){
         ...
         sshagent(['DOCKER_DEV_SERVER_SSH']) {
           sh "ssh -o StrictHostKeyChecking=no ubuntu@172.31.42.210 docker rm -f productmanager || true"
           sh "ssh -o StrictHostKeyChecking=no ubuntu@172.31.42.210 docker rmi 9871234/productmanager:${currentBuild.previousBuild.getNumber()} || true"
           sh "ssh -o StrictHostKeyChecking=no ubuntu@172.31.42.210 docker run -d -p 9090:8080 --name productmanager 9871234/productmanager:${buildNumber}"
            }
